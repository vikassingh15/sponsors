...
public interface CaseStatusChangeLogRepository extends CrudRepository<CaseStatusChangeLog, Long> {
    ...
    @Query(value = " SELECT DISTINCT x.candidate_id,x.diff, case_status.status " +
            " FROM (" +
            " SELECT caseChangeLog.candidate_id,  max(TIME_TO_SEC(caseChangeLog.end_time) - TIME_TO_SEC(caseChangeLog.start_time)) as diff" +
            ...
            " AND x.diff = TIME_TO_SEC(newChangeLog.end_time) - TIME_TO_SEC(newChangeLog.start_time)  ",
            nativeQuery = true)
